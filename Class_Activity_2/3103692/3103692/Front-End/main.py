...
<body>
  ...
  <div class="card">
    <h2>🧹 Limpiar Datos</h2>
    <button onclick='limpiarDatos()'>Limpiar sistema</button>
    <div id="clean-result" class="result"></div>
  </div>

  <div class="card">
    <h2>📄 Informe de Pruebas</h2>
    <button onclick='descargarInforme()'>Generar y descargar PDF</button>
  </div>
...
function limpiarDatos() {
  Promise.all([
    fetch('http://localhost:5001/clean', { method: 'POST' }),
    fetch('http://localhost:5002/clean', { method: 'POST' })
  ]).then(responses => Promise.all(responses.map(r => r.json())))
    .then(results => {
      const resultEl = document.getElementById('clean-result');
      if (results.every(r => r.status === 'ok')) {
        resultEl.textContent = '✅ Sistema limpio en ambos servicios';
        resultEl.className = 'result';
      } else {
        const errors = results.filter(r => r.status !== 'ok').map(r => r.details || r.error);
        resultEl.textContent = `❌ Errores: ${errors.join('; ')}`;
