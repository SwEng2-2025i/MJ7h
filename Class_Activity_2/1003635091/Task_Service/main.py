...
def create_task():
    ...
    try:
        user_check = requests.get(f'http://localhost:5002/users/{data["user_id"]}')
...
# ✅ ENDPOINT FALTANTE - Obtener tarea específica
...
# ✅ ENDPOINT FALTANTE - Eliminar tarea específica
...
# ✅ ENDPOINT FALTANTE - Eliminar tareas por usuario
@service_b.route('/tasks/user/<int:user_id>', methods=['DELETE'])
def delete_tasks_by_user(user_id):
    tasks_deleted = Task.query.filter_by(user_id=user_id).delete()
    db.session.commit()
    return jsonify({
        'message': f'Eliminadas {tasks_deleted} tareas del usuario {user_id}'
    }), 200

if __name__ == '__main__':
    with service_b.app_context():
        db.create_all()
    service_b.run(port=5003)
