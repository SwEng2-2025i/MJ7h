...
<body>
  ...
  <div class="card">
    ...
    <!-- Espacio para borrado -->
  <div class="card">
    <h2> 🗑 Borrar Tarea y Usuario </h2>
    <label>ID de Usuario:</label>
    <input id='userid_delete' placeholder='Ej: 1'>
    <button onclick='borrarUsuario()'>Borrar Usuario</button>
    <div id="user-delete-result" class="result"></div>

    <label> ID de tarea:</label>
    <input id='taskid_delete' placeholder='Ej: 1'>
...
function borrarUsuario() {
  const user_id = document.getElementById('userid_delete').value;
  console.log(user_id)
  fetch(`http://localhost:5001/users/${user_id}`, {
    method: 'DELETE',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify() // Va vacio nada en el body
  }).then(r => r.json()).then(data => {
    const result = document.getElementById('user-delete-result');
    if (data.id) {
      result.textContent = `✅ Usuario Borrado con ID ${data.id}`;
      result.className = 'result';
    } else {
      result.textContent = `❌ Error: ${data.error}`;
      result.className = 'error';
...
function borrarTarea() {
  const id = document.getElementById('taskid_delete').value;
  console.log(id)
  fetch(`http://localhost:5002/tasks/${id}`, {
    method: 'DELETE',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({id}) // Va vacio nada en el body
  }).then(r => r.json()).then(data => {
    const result = document.getElementById('task-delete-result');
    if (data.id) {
      result.textContent = `✅ Tarea Borrada con ID ${data.id}`;
      result.className = 'result';
    } else {
      result.textContent = `❌ Error: ${data.error}`;
      result.className = 'error';
