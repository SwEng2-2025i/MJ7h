...
<body>
    ...
    <div class="card">
        ...
        <button class="delete-button" onclick='eliminarTarea()'>Eliminar Tarea</button>
...
function eliminarUsuario() {
    const userIdToDelete = document.getElementById('delete-userid').value;
    fetch(`http://localhost:5001/users/${userIdToDelete}`, {
        method: 'DELETE'
    }).then(r => r.json()).then(data => {
...
function eliminarTarea() {
    const taskIdToDelete = document.getElementById('delete-taskid').value;
    fetch(`http://localhost:5002/tasks/${taskIdToDelete}`, {
        method: 'DELETE'
    }).then(r => r.json()).then(data => {
        ...
        if (data.message) {
            result.textContent = `✅ ${data.message}`;
            result.className = 'result';
            verTareas(); // Opcional: Actualizar la lista de tareas después de eliminar
        } else {
            result.textContent = `❌ Error: ${data.error}`;
            result.className = 'error';
        }
