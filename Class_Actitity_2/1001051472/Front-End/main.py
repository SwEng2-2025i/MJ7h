...
function eliminarUsuario() {
  const user_id = document.getElementById('delete-userid').value;
  fetch(`http://localhost:5001/users/${user_id}`, {
    ...
    if (res.status === 200) {
      result.textContent = ` ✅ Usuario eliminado correctamente`;
      result.className = 'result';
    } else {
      result.textContent = `❌ Error: ${res.data.error || 'No se pudo eliminar el usuario'}`;
      result.className = 'error';
    }
  });
}
...
function eliminarTarea() {
  const task_id = document.getElementById('delete-taskid').value;
  fetch(`http://localhost:5002/tasks/${task_id}`, {
    method: 'DELETE'
  }).then(r => r.json().then(data => ({status: r.status, data}))).then(res => {
    const result = document.getElementById('delete-task-result');
    if (res.status === 200) {
      result.textContent = res.data.message || `✅ Tarea eliminada correctamente`;
      result.className = 'result';
    } else {
      result.textContent = `❌ Error: ${res.data.error || 'No se pudo eliminar la tarea'}`;
      result.className = 'error';
    }
  });
}
